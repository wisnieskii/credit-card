...
function setCardType(type) {
    ...
    ccBgColor01.setAttribute('fiil', colors[type][0])
    ccBgColor02.setAttribute('fill', colors[type][1])
    ccLogo.setAttribute('src', `cc-${type}.svg`)
}
globalThis.setCardType = setCardType
const securityCode = document.querySelector('#security-code')
const securityCodePatterne = {
    mask: '0000',
}
const securityCodeMask = IMask(securityCode, securityCodePatterne)
...
cardHolder.addEventListener('input', () => {
    const ccHoler = document.querySelector('.cc-holder .value ')

    ccHoler.innerHTML = cardHolder.value.length === 0 ? 'FULANO DA SILVA' : cardHolder.value
})
...
function updateCardNumber(number) {
    ...
    ccInfo.innerHTML = number.length === 0 ? '1234 5678 9012 3456' : number
}
...
function updateExpirationDate(date) {
    ...
    ccExpiration.innerHTML = date.length === 0 ? '02/12' : number
}